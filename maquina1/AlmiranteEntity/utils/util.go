...
type RegisterInformante struct{
	NamePlanet string
	NameCity string
	RelojPlanet Reloj
	Server int64
...
func ContainsPlanet(slice []RegisterInformante, planeta string, ciudad string) int{
...
func PrintRegister(slice []RegisterInformante){
	for i, elemento := range slice{
		fmt.Println("\n--------- REGISTRO ", i+1, " -----------")
